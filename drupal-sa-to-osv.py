...
def project_dir_from_osv_entry(osv_dir_name, osv_entry):
  dir = os.path.join(
    osv_dir_name, osv_entry['affected'][0]['package']['name'].split('/')[1]
  )
...
def write_osv_entry_to_file(osv_dir_name, osv_entry, id):
  output_file = os.path.join(
    project_dir_from_osv_entry(osv_dir_name, osv_entry), f'osv-{id.lower()}.json'
  )
    ...
    for fixed_in in fixed_in_json:
      for fixed_version in fixed_in['list']:
        ...
        if fixed_in_semver in inserted:
          ...
          continue
        if (
          int(introduced_major) == int(fixed_major)
          and semver.compare(fixed_in_semver, introduced) > 0
        ):
    ...
    for file in files:
      if file.endswith('.json'):
        ...
        for ref in refs:
          if (
            'tags' in ref.keys()
            and 'x_refsource_CONFIRM' in ref['tags']
            and 'drupal.org/node/' in ref['url']
          ):
